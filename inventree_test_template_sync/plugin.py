...
class TestTemplateSyncPlugin(ActionMixin, UserInterfaceMixin, InvenTreePlugin):
    ...
    def get_ui_panels(self, request, context, **kwargs):
        """Add a synchronization panel to Part detail pages."""
        context = context or {}
        ...
        try:
            target = Part.objects.get(pk=target_id)
        except (Part.DoesNotExist, TypeError, ValueError):
            return []

        # The sync backend requires the target Part to be testable.
        # Hiding the panel on non-testable Parts keeps the UI focused.
        if not target.testable:
            return []

        return [
            {
                "key": "test-template-sync-panel",
                "title": "Test Template Sync",
                "description": (
                    "Synchronize test templates from another Part while preserving history."
                ),
                "icon": "ti:test-pipe:outline",
                "source": self.plugin_static_file(
                    "test_template_sync.js:renderTestTemplateSyncPanel"
                ),
                "context": {
                    "target_part": target.pk,
                    "target_name": target.name,
                    "target_ipn": target.IPN or "",
    ...
    def _sync(self, user, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for key in stale_keys:
            ...
            if disable_stale and stale_template.enabled and dry_run:
